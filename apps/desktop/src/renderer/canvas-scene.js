/**
 * Ryloom desktop — Canvas scene engine (renderer, CommonJS).
 *
 * A faithful port of apps/web/src/lib/recorder/canvas-scene.ts so the desktop
 * canvas stage renders pixel-identically to the browser recorder: the same 6
 * background painters, the same 6 text layouts (normalized slot geometry), the
 * same 6 one-tap templates, and the same compositor math. Keep the two in sync.
 *
 * A canvas is a full-frame designed page: a painted background plus an editable
 * text layout your camera bubble floats over. On desktop the stage is a real
 * always-on-top window the screen recording captures live (canvas.html).
 */
"use strict";
...
// ---------------------------------------------------------------------------
// Text layouts — normalized slot geometry shared by the DOM stage + compositor.
//   x/w are fractions of stage WIDTH, y of stage HEIGHT; `size` is a fraction
//   of stage WIDTH so the DOM `vw` font size equals canvas.width * size.
// ---------------------------------------------------------------------------
...
function canvasInkColor(background) {
  const preset = CANVAS_BACKGROUND_PRESETS.find((p) => p.id === background);
  return preset && preset.ink === "dark" ? "#241f33" : "#ffffff";
}
...
function flower(ctx, x, y, size, petal, center, rotation = 0) {
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(rotation);
  ctx.fillStyle = petal;
...
const CANVAS_BACKGROUND_PAINTERS = {
  gradient(ctx, w, h) {
...
const CANVAS_TEMPLATES = [
  ...
  },
  {
    id: "bubble",
    label: "Bubble",
    preset: { background: "flowers", layout: "empty", texts: {} },
    centersBubble: true,
  },
];

/** Builds the scene a template represents (template "none" disables). */
function applyCanvasTemplate(scene, template) {
  if (!template.preset) {
...
/** Draws background + text slots. Any camera bubble is drawn afterwards. */
function paintCanvasScene(ctx, w, h, scene) {
